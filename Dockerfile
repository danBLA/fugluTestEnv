...
RUN yum  -y install clamav clamav-scanner clamav-update clamav-data spamassassin python-magic git python-setuptools python-nose unar python-sqlalchemy python-pip postfix python34 python34-nose python34-pip libffi-devel python-devel python-pip ssdeep-devel ssdeep-libs python34-devel wget libxml2
...
RUN pip2 install rarfile mock dkimpy dnspython pydns pyspf==2.0.12t ipaddr rednose ssdeep==3.2 redis geoip2 beautifulsoup4 lxml pysrs
RUN pip3 install --upgrade pip 'setuptools>=20.2.2'
RUN pip3 install rarfile rednose  sqlalchemy python-magic pyspf py3dns mock ssdeep redis geoip2 beautifulsoup4 lxml pysrs
